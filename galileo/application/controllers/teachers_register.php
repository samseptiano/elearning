...
class teachers_register extends CI_Controller
{
	...
	function check_availability_username(){
		...
		if($username != "" && $username != $this->session->userdata('nama')){
		  ...
		  $user_count = $this->m_register->checkAvailData("data_guru",$where)->num_rows();
	...
	function check_availability_email(){
	 ...
	 private function cekAvailable(){
		...
		$cek = $this->m_register->checkAvailData("data_guru",$where)->num_rows();
		return $cek;
	}
    ...
    function register()
    {
		...
		else
		{
			...
			if($cek < 1){
				if ($this->m_register->insertUser("data_guru",$data))
				{
					// send email
