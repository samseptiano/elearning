...
<html lang="en">
  <head>
    ...
    <title>Elearning Galileo | Chat Siswa</title>
  ...
  <script> 
    ...
    var updateTime = function (cb) {
      $.getJSON("<?php echo base_url(); ?>/Students/time", function (data) {
          cb(~~data);
      });
    };
    
    var sendChat = function (message,sender,receiver, cb) {
      $.getJSON("<?php echo base_url(); ?>/Students/insert_chat?message=" + message+"&sender=" + sender+"&receiver=" + receiver, function (data){
        cb();
      });
    }
    ...
    var addDataToReceived = function (arrayOfData) {
      arrayOfData.forEach(function (data) {
        ...
        if(data[0]== sender){
                  $("#received").html($("#received").html() + "<br><h5 style='color:red;margin-right:0;' >" + "Saya" +" : "+ data[1]+"</h5> <p style='font-size=10px'>("+ dateString +")</p>");
        }
    ...
    var getNewChats = function (sender,receiver) {
      $.getJSON("<?php echo base_url(); ?>Students/get_chats?time=" + time+"&sender=" + sender +"&receiver=" + receiver, function (data){
...
<div class="col-sm-6 form-group">
  <input id="sender" type="hidden" name="sender"  value="<?php echo $this->session->userdata("id"); ?>" class="form-control">
          ...
          <input name="receiver" id="receiver" class="form-control" value="<?php echo $u->id_tutor ?>" type="hidden">
          <?php } ?>
  ...
  <div class="col-sm-12 form-group" style="background-color: #ffffff;opacity: 0.8;box-shadow: 5px 5px 10px #888888;border-radius:5px;overflow-x: auto;padding-top:10px">
      <p id="received" style="height: 350px;overflow: auto;" class="data"></p>
  </div>
                ...
                setInterval(function()
                  { 
                    var a = document.getElementById("receiver").value;
                      $.ajax({
                        url:"<?php echo base_url(); ?>"+"students/keep_read",
                        async: false,
                        type:"post",
                        dataType: "json",
                        data: {sender: a},
