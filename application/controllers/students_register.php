...
class students_register extends CI_Controller
{
	...
	function check_availability_username(){
		...
		if($username != ""){
		  ...
		  $user_count = $this->m_register->checkAvailData("siswa",$where)->num_rows();
	...
	function check_availability_email(){
	 ...
	 private function cekAvailable(){
		...
		$cek = $this->m_register->checkAvailData("siswa",$where)->num_rows();
		return $cek;
	}
    ...
    function register()
    {
    	$expl = explode(" ",$this->input->post('grade'));
			...
			if($cek < 1){
				if ($this->m_register->insertUser("siswa",$data))
				{
					// send email
